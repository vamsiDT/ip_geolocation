for I in $(cat ips.txt|head -n2|tail -n1);do
for J in $(cat working_nodes.dat);do
a=$(ssh -o "StrictHostKeyChecking no" -l upmc_netmet $J sudo ping -c 10 -i 0.000001 $I > /tmp/xc; cat /tmp/xc |tail -n1 | awk '{print $4}' | awk -F "/" '{print $1}')
#if [ $a -gt 0 ];then
echo -e "node:\t$J\tminrtt:\t$a"
#fi
#IP=$I
#echo "$IP"
